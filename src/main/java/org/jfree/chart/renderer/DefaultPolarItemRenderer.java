...
public class DefaultPolarItemRenderer extends AbstractRenderer
        ...
        if (this.shapesVisible) {
            ...
            while (!pi.isDone()) {
                ...
                final Shape shape = ShapeUtils.createTranslatedShape(
                        getShape().getItemShape(seriesIndex, i++), x,  y);

                Paint paint;
                if (useFillPaint) {
                    paint = getPaint().lookupSeriesFillPaint(seriesIndex);
                }
                else {
                    paint = getPaint().lookupSeriesPaint(seriesIndex);
                }
                g2.setPaint(paint);
                g2.fill(shape);
    ...
    public LegendItem getLegendItem(int series) {
        ...
        Shape shape = getShape().lookupSeriesShape(series);
        Paint paint;
        if (this.useFillPaint) {
            paint = getPaint().lookupSeriesFillPaint(series);
        }
        else {
